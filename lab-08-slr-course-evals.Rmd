...
***Yes, the data is skewed. The lower scores compared to the higher are very small, which could result in students having a more favorable outlook on the course. I would expect students to rank classes higher.***
...
evals %>%
  ...
  ylab("Average Professor Evalutation") +
  ggtitle("Scatter plot of Average Beauty Rating to Average Evaluation Score")

```

***There is a slight increase of evaluation score due to beauty rating. It is not super high, but it creates a positive relationship***
...
***The initial plot did not show how many dots landed on a specific point, so it was misleading*** 
...
***For an average beauty rating increased by 1 and the average evaluation rating goes up by 0.07***
...
***For an average beauty rating of 0, the average evaluation rating is 3.88. This doesn't make sense in the data as there are no beauty rating that are 0***
...
$$\widehat{score}-i = 4.28 - 0.12\times TenureTrack_i - 0.14\times Tenured_i$$
...
$$\wide{score}= 4.14 + 0.14\times teaching + 0.02\times Tenure tracks$$
